...
export default function DonorManagement({ donors, onUpdate }: DonorManagementProps) {
  ...
  const filteredDonors = donors.filter(donor => {
    ...
    const matchesBloodGroup = filterBloodGroup === 'all' || !filterBloodGroup || donor.blood_group === filterBloodGroup;
  ...
  const handleAddDonor = async (formData: FormData) => {
    ...
    try {
      const newDonor = {
        ...
        hospital_id: 'h1', // Current hospital
        name: formData.get('name') as string,
        age: parseInt(formData.get('age') as string),
        blood_group: formData.get('bloodGroup') as BloodGroup,
      ...
      await supabaseApi.createDonor(newDonor);
                ...
                <TableRow key={donor.id}>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <Badge 
                        variant={donor.is_active ? "default" : "secondary"}
                        className={donor.is_active ? "bg-green-100 text-green-800" : ""}
                      >
                        {donor.is_active ? 'Active' : 'Inactive'}
                      </Badge>
                      <Badge 
                        variant={donor.verification_status === 'verified' ? "default" : "outline"}
                        className={donor.verification_status === 'verified' ? "bg-blue-100 text-blue-800" : ""}
                      >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{selectedDonor.contact_number}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm">{selectedDonor.medical_history || 'No medical history provided'}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Status</Label>
                  <div className="space-y-1">
                    <Badge variant={selectedDonor.is_active ? "default" : "secondary"}>
                ...
                <div>
                  ...
                  <Badge variant={selectedDonor.verification_status === 'verified' ? "default" : "outline"}>
                    {selectedDonor.verification_status}
                  </Badge>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {new Date(selectedDonor.updated_at || selectedDonor.created_at).toLocaleDateString()}
                </p>
              </div>
            </div>
          </DialogContent>
