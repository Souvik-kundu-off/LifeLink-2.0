...
export default function RecipientManagement({ recipients, onUpdate }: RecipientManagementProps) {
  ...
  const filteredRecipients = recipients.filter(recipient => {
    const matchesSearch = recipient.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         recipient.contact_number?.includes(searchTerm);
    const matchesUrgency = filterUrgency === 'all' || !filterUrgency || recipient.urgency_level === filterUrgency;
  ...
  const handleAddRecipient = async (formData: FormData) => {
    ...
    try {
      const newRecipient = {
        ...
        urgency_level: formData.get('urgencyLevel') as UrgencyLevel,
      ...
      await supabaseApi.createRecipient(newRecipient);
