...
export default function AlertSystem({ donors, recipients }: AlertSystemProps) {
  ...
  const criticalRecipients = recipients.filter(r => (r.urgencyLevel || r.urgency_level) === 'critical' && r.status === 'waiting');
  ...
  const calculateEstimatedReach = (formData: FormData) => {
    ...
    const compatibleDonors = donors.filter(donor => {
      const bloodMatch = selectedBloodGroups.length === 0 || selectedBloodGroups.includes(donor.bloodGroup || donor.blood_group);
      // Mock distance calculation (in real app, would use geospatial queries)
      const withinRadius = true; // Simplified for demo
      
      return bloodMatch && (donor.isActive || donor.is_active) && withinRadius;
    });
                  ...
                  <SelectContent>
                    ...
                    {recipients.filter(r => r.status === 'waiting').map(recipient => (
                      <SelectItem key={recipient.id} value={recipient.id}>
                        {recipient.name} - {recipient.bloodGroup || recipient.blood_group} - Blood donation needed
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-red-700 mt-1">
                        ...
                        <Badge variant="outline" className="font-mono text-red-800 border-red-300">
                          {recipient.bloodGroup || recipient.blood_group}
                        </Badge>
                        <span>Needs: Blood donation</span>
                      </div>
                      <div className="flex items-center space-x-1 text-xs text-red-600 mt-1">
                        <Clock className="h-3 w-3" />
                        <span>Waiting {Math.floor((Date.now() - new Date(recipient.registrationDate || recipient.created_at).getTime()) / (1000 * 60 * 60 * 24))} days</span>
                      </div>
                    </div>
                    <Button 
                      size="sm" 
